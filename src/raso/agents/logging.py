...
from config.backend.models import RASOMasterState, AgentType, WorkflowStatus
...
class AgentLogger:
    ...
    def __init__(self, agent_type: Optional[AgentType], job_id: Optional[str] = None):
        """
        Initialize agent logger.
        
        Args:
            agent_type: Type of agent (can be None for service loggers)
        ...
        # Create logger
        logger_name = f"raso.agents.{agent_type.value}" if agent_type else "raso.service"
    ...
    def _setup_handlers(self) -> None:
        ...
        if self.config.env == "production":
            ...
            agent_name = self.agent_type.value if self.agent_type else "service"
        ...
        if self.config.log_path:
            agent_name = self.agent_type.value if self.agent_type else "service"
            log_file = self.config.log_path / f"{agent_name}.log"
    ...
    def _get_context(self) -> Dict[str, Any]:
        """Get logging context."""
        context = {
            "agent_type": self.agent_type.value if self.agent_type else "service",
        }
