#!/usr/bin/env python3
"""
Fix the second while loop that's generating the repetitive scenes
"""

def fix_second_while_loop():
    """Fix the second while loop with diverse scene templates."""
    
    # Read the current file
    with open('production_video_generator.py', 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Find and replace the second while loop section
    old_second_while = '''        # If total duration is less than 900 seconds (15 minutes) OR less than 10 scenes, add more scenes
...
""",
    ...
    new_second_while = '''        # Create diverse additional comprehensive scenes with 300+ words each (second loop)
        scene_templates_2 = [
            {
        ...
        # Add diverse scenes to reach minimum 10 scenes
        scene_count_2 = 0
        while (total_duration < 900 or len(scenes) < 10) and len(scenes) < 20:  # Limit to prevent infinite loop
            template_index = scene_count_2 % len(scene_templates_2)
            template = scene_templates_2[template_index]
...
""",
    ...
    content = content.replace(old_second_while, new_second_while)
    
    # Write the enhanced content back
    with open('production_video_generator.py', 'w', encoding='utf-8') as f:
        f.write(content)
    
    print("✅ Fixed second while loop with:")
...
if __name__ == "__main__":
    success = fix_second_while_loop()
    if success:
        print("✅ Second while loop fix completed!")
    else:
        print("❌ Failed to fix second while loop")